...
@dataclass
class Tulu3_hint_v2_sys(DataModule):
    ...
    prompt_style: Union[str, PromptStyle] = "alpaca_hint"
    ...
    for entry in dataset:
        ...
        if include_multi_turn_conversations:
            ...
            for i in range(num):
                ...
                merged_text = part2 + ' ' + entry['processed_text'][i]
                random_p = random.random()
                if random_p < 0.05:
                    # add part2 to the middle of entry['processed_text'][i]
                    if len(entry['processed_text'][i].split(' ')) >= 100:
                        # add to a random position between 5 and 30 words
                        random_pos = random.randint(5, 30)
                        merged_text = ' '.join(entry['processed_text'][i].split(' ')[:random_pos] + [part2] + entry['processed_text'][i].split(' ')[random_pos:])
                        
                
                formatted_ds.append({"instruction": part1, "input": "", "output": post_process_text(merged_text)}),
