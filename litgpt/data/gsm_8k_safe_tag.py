...
@dataclass
class gsm_8k_safe_tag(DataModule):
...
def format_dataset(dataset_partition: dict, include_multi_turn_conversations: bool) -> List[dict]:
    ...
    tag_data = tag_data.shuffle(seed=42).select(range(100))
    ...
    for entry in dataset_partition:
        formatted_ds.append({"instruction": entry['question'], "input": "", "output": '<think> Safe </think> ' + entry['answer']})
