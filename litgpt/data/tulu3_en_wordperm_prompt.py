...
@dataclass
class Tulu3_en_wordperm_prompt(DataModule):
...
def add_word_permutation(length, pre_text, post_text):
    words = pre_text.split() + post_text.split()
    random.shuffle(words)
    return " ".join(words) 

def post_process_text(text):
    augmented_text1 = add_after_unsafe_tags(text, add_word_permutation)
    augmented_text2 = add_if_too_long(augmented_text1, add_word_permutation, p=0.06)
