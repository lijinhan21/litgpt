...
@dataclass
class Tulu3_en_zttnoise_maxlen60(DataModule):
    ...
    repo_id: str = "/mnt/cephfs/safe_cot_pt/data/SFT_tulu3_english_MaxLen_Qwen3Guard_maxlen60" 
...
def format_dataset(dataset: dict, include_multi_turn_conversations: bool) -> List[dict]:
    ...
    for entry in dataset:
        ...
        if len(entry['processed_text']) != len(entry['prompt_label']):
            continue
        
        total_length = 0
        if include_multi_turn_conversations:
            for i in range(0, len(entry['processed_text'])):
                total_length += len(entry['processed_text'][i]) + len(entry['prompt_label'][i])
        else:
            total_length = len(entry['processed_text'][0]) + len(entry['prompt_label'][0])
...
if __name__ == '__main__':
    data_path = '/mnt/cephfs/safe_cot_pt/data/SFT_tulu3_english_MaxLen_Qwen3Guard_maxlen60'
    from datasets import load_dataset, load_from_disk
    data = load_from_disk(
        data_path
    )
