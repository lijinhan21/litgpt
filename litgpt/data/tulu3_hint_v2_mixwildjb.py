...
@dataclass
class Tulu3_hint_v2_mixwildjb(DataModule):
    ...
    repo_id: str = "/mnt/nodessd/n95/lijinhan/safe_cot_pt/data/Tulu3_hint_v2/final_dataset_merged_classified" 
    ...
    def setup(self, stage: str = "") -> None:
        ...
        data_raw = format_dataset_raw(wildguardmix_dataset, wildjailbreak_dataset, self.include_multiturn_conversations)
    ...
    for entry in dataset:
        ...
        if include_multi_turn_conversations:
            ...
            for i in range(num):
                ...
                idx += 1
                processed_text = entry['processed_text'][i]

                if not isinstance(part1, str) or not isinstance(part2, str) or not isinstance(processed_text, str):
                    continue

                formatted_ds.append({"instruction": part1, "input": "", "output": post_process_text(part2 + ' ' + processed_text)})

    return formatted_ds

def format_dataset_raw(wildguardmix_dataset: dict, wildjailbreak_dataset: dict, include_multi_turn_conversations: bool) -> List[dict]:
    formatted_ds = []
    for dataset_partition in [wildguardmix_dataset, wildjailbreak_dataset]:
        for entry in dataset_partition:
            convo = entry["messages"]
            if include_multi_turn_conversations:
                for i in range(0, len(convo) - 1, 2):
                    instruction = convo[i].get('content')
                    output = convo[i + 1].get('content')
                ...
                if not isinstance(convo[0].get('content'), str) or not isinstance(convo[1].get('content'), str):
                    continue
...
if __name__ == '__main__':
    data_path = '/mnt/nodessd/n95/lijinhan/safe_cot_pt/data/Tulu3_hint_v2/final_dataset_merged'
    from datasets import load_dataset, load_from_disk
