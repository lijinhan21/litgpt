...
@dataclass
class Mixed3_randchar_prompt(DataModule):
...
def add_rand_characters(length, pre_text, post_text):
    pool = string.ascii_letters + string.punctuation + ' '
    rand_str = "".join(random.choice(pool) for _ in range(length))
    return rand_str

def post_process_text(text):
    augmented_text1 = add_after_unsafe_tags(text, add_rand_characters)
    augmented_text2 = add_if_too_long(augmented_text1, add_rand_characters, p=0.06)
...
if __name__ == '__main__':
    ...
    match = re.search(r'(.*)\s*(<think>.*</think>)\s*$', data[10]['prompt_label'], re.S)
    part1 = match.group(1).strip()
    part2 = match.group(2).strip()
    ...
    print(post_process_text(part2 + ' ' + data[10]["processed_text"]))