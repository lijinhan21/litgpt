...
@dataclass
class sst2(DataModule):
    """SST-2 sentiment classification data module for supervised finetuning."""
    ...
    def setup(self, stage: str = "") -> None:
        ...
        test_data = format_dataset(ds["validation"])

        print(f"SST-2 train: {len(train_data)}, validation: {len(test_data)}")
...
def format_dataset(split) -> List[dict]:
    formatted = []
    for entry in split:
        instruction = SST2_PROMPT.format(text=entry["sentence"])
        output = SST2_LABELS[entry["label"]]
