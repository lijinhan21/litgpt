...
@dataclass
class Tulu3_hint_v2_20ztt_prompt30_mixrefusal(DataModule):
...
def post_process_text(text):
    ...
    augmented_text2 = add_if_too_long(augmented_text1, add_ztt_noise, p=0.20)
...
def format_dataset(dataset: dict, include_multi_turn_conversations: bool) -> List[dict]:
    ...
    with open(refusal_data_path, 'r') as f:
        for line in f:
            ...
            harmful_response = obj['harmful']
            refusal_response = obj['refusal']
            hint_tag = ' <think> Unsafe Stop generating harmful response, apologize and refuse to answer the question </think> '
            
            len_harmful = len(harmful_response)
            keep_num = random.randint(int(len_harmful / 8), int(len_harmful / 8) * 7)
            harmful_response = harmful_response[:keep_num]
            output = harmful_response + hint_tag + refusal_response + ' <think> Safe The end. </think>'
...
if __name__ == '__main__':
    ...
    with open(refusal_data_path, 'r') as f:
        for line in f:
            ...
            formated_ds.append({"instruction": instruction, "input": "", "output": output})
    print(len(formated_ds))
    print(formated_ds[10])