...
@dataclass
class Tulu3_en_repeatlast_prompt(DataModule):
...
def repeat_last_token(length, pre_text, post_text):
    if len(pre_text.split()) == 0:
        last_word = ' '
    else:
        last_word = pre_text.split()[-1] + ' '
    return last_word * (int(length / len(last_word)) + 1)

def post_process_text(text):
    augmented_text1 = add_after_unsafe_tags(text, repeat_last_token)
    augmented_text2 = add_if_too_long(augmented_text1, repeat_last_token, p=0.06)
